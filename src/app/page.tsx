...
function OfflineStatusIndicator() {
  ...
  // Derive pending count reactively from the queue state
...
// Module card data for the dashboard (static, defined outside component)
...
// Dashboard component extracted outside FrigorificoApp to prevent remounts on every re-render
function DashboardContent({ operador, setCurrentPage, stats, tropas, actividadReciente, hasPermission }: DashboardContentProps) {
...
export default function FrigorificoApp() {
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      case 'dashboard':
        return <DashboardContent operador={operador} setCurrentPage={setCurrentPage} stats={stats} tropas={tropas} actividadReciente={actividadReciente} hasPermission={hasPermission} />
      case 'pesajeCamiones':
